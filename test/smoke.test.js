...
test('미국 메가캡은 자동 프로필 정보가 응답에 포함된다', async () => {
  const { response, body } = await getJson('/api/stock/technical?ticker=AAPL&range=6mo&suffix=none');

  assert.equal(response.status, 200);
  assert.equal(body.ok, true);
  assert.equal(body.data.profile.key, 'us_megacap_growth');
  assert.equal(body.data.profile.market, 'US');
});
...
test('historical snapshot은 range 밖 날짜 요청 시 500 대신 명시적 오류를 반환한다', async () => {
  const { response, body } = await getJson('/api/stock/historical-snapshot?ticker=AAPL&snapshot_date=2024-01-02&range=2y&suffix=none');

  assert.equal(response.status, 404);
  assert.equal(body.ok, false);
  assert.equal(body.error.code, 'NO_DATA_IN_RANGE');
  assert.match(body.error.message, /range/);
});
...
test('공개 소개 페이지가 정상적으로 열린다', async () => {
  const { response, body } = await getText('/about');

  assert.equal(response.status, 200);
  assert.match(body, /Stock Analyzer/);
  assert.match(body, /About|소개/);
});

test('공개 문의 페이지가 정상적으로 열린다', async () => {
  const { response, body } = await getText('/contact');

  assert.equal(response.status, 200);
  assert.match(body, /Contact|문의/);
});

test('개인정보처리방침 페이지가 정상적으로 열린다', async () => {
  const { response, body } = await getText('/privacy');

  assert.equal(response.status, 200);
  assert.match(body, /개인정보처리방침|Privacy/);
});
