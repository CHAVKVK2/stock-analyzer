...
function buildReasons(ctx, marketState, signal) {
  if (signal === 'NEUTRAL') {
    return ['Signals are mixed, so the setup is better treated as a watchlist candidate.'];
  }

  const reasons = [];

  if (signal === 'BUY') {
    if (marketState.trend === 'uptrend') reasons.push('EMA trend alignment is supportive of an uptrend.');
    if (crossedAbove(ctx.prevMacd, ctx.prevMacdSignal, ctx.macd, ctx.macdSignal)) reasons.push('MACD has recently crossed bullish.');
    if (isNumber(ctx.volumeRatio) && ctx.volumeRatio >= 1.5) reasons.push(`Volume is ${ctx.volumeRatio.toFixed(2)}x the 20-day average.`);
    if (isNumber(ctx.adx) && ctx.adx > 25) reasons.push('ADX confirms that the trend has enough strength.');
    if (isNumber(ctx.nearestSupport) && isNumber(ctx.lastClose) && percentDistance(ctx.lastClose, ctx.nearestSupport) <= 0.03) reasons.push('Price is holding close to a recent support zone.');
  } else {
    if (marketState.trend === 'downtrend') reasons.push('EMA trend alignment is supportive of a downtrend.');
    if (crossedBelow(ctx.prevMacd, ctx.prevMacdSignal, ctx.macd, ctx.macdSignal)) reasons.push('MACD has recently crossed bearish.');
    if (isNumber(ctx.volumeRatio) && ctx.volumeRatio >= 1.5) reasons.push(`Down move is backed by elevated volume at ${ctx.volumeRatio.toFixed(2)}x average.`);
    if (isNumber(ctx.adx) && ctx.adx > 25) reasons.push('ADX confirms that the downtrend has strength.');
    if (isNumber(ctx.nearestSupport) && isNumber(ctx.lastClose) && ctx.lastClose < ctx.nearestSupport) reasons.push('Price has broken below a nearby support level.');
  }
...
function buildRisks(ctx, marketState, signal) {
  ...
  if (isNumber(ctx.atr) && isNumber(ctx.lastClose)) {
    const atrRatio = ctx.atr / ctx.lastClose;
    if (atrRatio >= 0.05) risks.push('ATR is elevated, so volatility is higher than usual.');
  }
  ...
  if (signal === 'BUY') {
    if (isNumber(ctx.nearestResistance) && isNumber(ctx.lastClose) && percentDistance(ctx.nearestResistance, ctx.lastClose) <= 0.03 && ctx.lastClose < ctx.nearestResistance) {
      risks.push('Price is still close to a recent resistance zone.');
    }
    if (isNumber(ctx.rsi) && ctx.rsi > 70) risks.push('RSI is near overbought territory, so upside may be crowded.');
  } else if (signal === 'SELL') {
    if (isNumber(ctx.nearestSupport) && isNumber(ctx.lastClose) && percentDistance(ctx.lastClose, ctx.nearestSupport) <= 0.03 && ctx.lastClose > ctx.nearestSupport) {
      risks.push('Price is close to support, so a bounce risk remains.');
    }
    if (isNumber(ctx.rsi) && ctx.rsi < 30) risks.push('RSI is near oversold territory, so a short-covering bounce is possible.');
  } else {
    if (marketState.trendStrength === 'weak') risks.push('Trend strength is weak, which increases false-signal risk.');
  }

  if (!risks.length) {
    risks.push('No major technical risk stands out beyond normal market noise.');
  }
